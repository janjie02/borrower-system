    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-[#6B7280]">{profile.profiles?.email}</p>
        </div>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <dl className="grid grid-cols-2 gap-3 text-sm">
              <div>
                <dt className="text-[#6B7280]">ID Code</dt>
              ...
              {profile.account_type === "student" && (
                <>
                  <div>
                    <dt className="text-[#6B7280]">Year</dt>
                    <dd className="font-medium">{profile.year ?? "—"}</dd>
                  </div>
                  <div>
                    <dt className="text-[#6B7280]">Section</dt>
              ...
              <div>
                <dt className="text-[#6B7280]">Credit Status</dt>
        ...
        <Card>
          ...
          <CardContent>
            {recentRequests.length === 0 ? (
              <p className="text-sm text-[#6B7280]">No borrowing history yet</p>
            ) : (
              <ul className="divide-y divide-[#E5E7EB]">
                {recentRequests.map((req) => (
                  <li key={req.id} className="flex items-center justify-between py-2 text-sm">
                    <span className="font-mono text-[#1565C0]">{req.request_number}</span>
      ...
      <Card>
        ...
        <CardContent>
          {creditHistory.length === 0 ? (
            <p className="text-sm text-[#6B7280]">No credit transactions yet</p>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead className="border-b border-[#E5E7EB]">
                  <tr>
                    ...
                    <th className="py-2 text-left font-medium text-[#6B7280]">Behavior</th>
                    <th className="py-2 text-right font-medium text-[#6B7280]">Change</th>
                    <th className="py-2 text-right font-medium text-[#6B7280]">New Score</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#E5E7EB]">
                  {creditHistory.map((entry) => (
                    <tr key={entry.id}>
                      <td className="py-2 text-[#6B7280]">{formatDateTime(entry.created_at)}</td>
                      ...
                      <td
                        className={`py-2 text-right font-semibold ${
                          entry.credit_change >= 0 ? "text-green-700" : "text-red-700"
                        }`}
                      >
